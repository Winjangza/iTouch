...
void Database::updateIpNetwork(const QString &newUser, const QString &newIpMaster, const QString &newIpSlave,const QString &snmp ,const QString &timeserver){
    if (!db.isOpen()) {
        qDebug() << "Database not open, attempting to open...";
        if (!db.open()) {
            qWarning() << "Failed to open database:" << db.lastError().text();
    ...
    QSqlQuery query;
    query.prepare("UPDATE userMode SET USER = :user, IP_MASTER = :ipMaster, IP_SLAVE = :ipSlave, IP_SNMP = :snmp , IP_TIMERSERVER = :timeserver  WHERE id = 1");
...
void Database::UpdateStatusOfButtonHidding(QString msg) {
    ...
    QString updateSQL = QString("UPDATE updateButtonHiding SET %1 WHERE id = 1").arg(updateParts.join(", "));
    QSqlQuery query(db);
